...
/*
 * bpf_esp_map
 * 	Map a physical address *phys_addr* with partition size *size*.
 *
 * Returns
 	0 on success. *addr_mem* contains the virtual mapping.
 */
static long (* const bpf_esp_map)(void *addr_mem, __u32 size, const void *phys_addr) = (void *) 212;

/*
 * bpf_esp_unmap
 * 	Remove mapping to *virt_addr*.
 *
 * Returns
 	0 on success. *addr_mem* contains the virtual mapping.
 */
static long (* const bpf_esp_unmap)(void *addr_mem, __u32 size, void *virt_addr) = (void *) 213;


/*
 * bpf_esp_read
 * 	Read a 32-bit value from *unsafe_ptr* into *dst* using ioread32.
 */
static long (* const bpf_esp_read)(void *dst, __u32 size, const void *unsafe_ptr) = (void *) 214;

/*
 * bpf_esp_write
 * 	Write a 32-bit value from *val* into *dst* using ioread32.
 */
static long (* const bpf_esp_write)(void *val, __u32 size, const void *unsafe_ptr) = (void *) 215;
